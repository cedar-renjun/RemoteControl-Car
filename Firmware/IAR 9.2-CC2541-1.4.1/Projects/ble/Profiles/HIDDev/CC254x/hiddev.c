  ...
  if ( events & HID_SEND_REPORT_EVT )
  {
    ...
    if ( events & HID_STATUS_EVT )
    {
        ...
        */
        ...
        if(GAPROLE_INIT == hidDevGapState)
        {
            // Power On
            // Led ON 1 s
            HalLedSet( HAL_LED_1, HAL_LED_MODE_ON);
            osal_start_timerEx( hidDevTaskId, HID_STATUS_EVT, 1000 );
        }
        else if(GAPROLE_ADVERTISING == hidDevGapState)
        {
            HalLedBlink( HAL_LED_1, 2, 30, 100 );
            osal_start_timerEx( hidDevTaskId, HID_STATUS_EVT, 1000 );
        }
        else if(GAPROLE_CONNECTED == hidDevGapState)
        {
            HalLedSet( HAL_LED_1, HAL_LED_MODE_ON);
        }
        else
...
static void hidDevGapStateCB( gaprole_States_t newState )
{
  ...
  // Update LEDs
  osal_start_timerEx( hidDevTaskId, HID_STATUS_EVT, 20 );
}
